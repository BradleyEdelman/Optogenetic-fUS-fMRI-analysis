...
close all
clear all
folder = '/oak/stanford/groups/ljinhy/bjedelma/US_IMAGING/20-August-2018/';

fileA ={'IQdata_20-August-2018_16-17-32.mat';...
    'IQdata_20-August-2018_16-45-58.mat';...
    };
...
fileF = {'IQdata_20-August-2018_16-17-32_RHL_STIM_10uA_DECENT.mat';...
    'IQdata_20-August-2018_16-45-58_RHL_STIM_10uA_DECENT.mat';...
    };
...
%             C{k}(i,j) = STATS(1);
    ...
    figure(101);
    pdiA2{k}=pdiA{end}(1:2:end,1:2:end)-1;
    pdiA2{k}(cthresh{k} ~= 0) = cthresh{k}(cthresh{k} ~= 0);
    subplot(size(pdiF,2),1,k); imagesc(pdiA2{k})
    title(['R-value: Run ' time{k}])
    caxis([-26 1]);CAX = caxis; Cbar=[gray(abs(CAX(1)-1)*100);jet(abs(CAX(2)+1)*100)]; colormap(Cbar); 
...
pdiA22 = pdiA{end}(1:2:end,1:2:end);
...
for k = 1:size(pdiF,2)
    ...
    pdiA3{k}=pdiA{end}(1:2:end,1:2:end)-1;
