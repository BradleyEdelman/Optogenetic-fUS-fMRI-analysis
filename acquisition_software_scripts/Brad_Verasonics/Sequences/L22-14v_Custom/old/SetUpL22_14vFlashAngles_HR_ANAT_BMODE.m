...
Resource.ImageBuffer(1).numFrames = 30;
...
TW.type = 'parametric';
TW.Parameters = [Trans.frequency, 0.67, 2, 1];   % 18 MHz center frequency, 67% pulsewidth 1.5 cycle burst
...
for i=1:na*pgs
    if na>1
        ...
        for j = 1:na  % For each row in the column
            ...
            ReconInfo(j).pagenum = i;
        end
        
        if isequal(rem(i,na),0)
...
SeqControl(2).argument = 1/(PRF*1e-06);
...
for i=1:Resource.RcvBuffer(1).numFrames
    for k=1:pgs
        ...
        if floor(i/na) == i/na     % Exit to Matlab every 5th frame
