...
if (na > 1)
    dtheta2D = (6*pi/180)/(na-1);
    startAngle = -6*pi/180/2;
else
    dtheta2D = 0;
    startAngle=0;
...
ne = na*50;     % Set ne = number of acquisitions in Doppler ensemble.
dopPRF = na*500; % Doppler PRF in Hz.
pwrThres = 0.3;
...
Media.MP(2,:) = [0,0,25,1.0];
Media.MP(3,:) = [50,0,100,2.0];
Media.MP(4,:) = [-50,0,50,4.0];
Media.numPoints=4;
...
Resource.RcvBuffer(1).numFrames = 5;           % x frames allocated for RF acqusitions.
% InterBuffer(1) is for Doppler reconstructions.
Resource.InterBuffer(1).numFrames = 200;          % one intermediate frame needed for Doppler.
Resource.InterBuffer(1).pagesPerFrame = na;     % ne pages per ensemble
...
TX = repmat(struct('waveform', 1, ...
                   ...
                   'Delay', zeros(1,Trans.numelements)), 1, na); % na TXs for Doppler
...
% These will be referenced in the same event, so that they will use the 
% same (most recent) acquisition frame.
...
ReconInfo = repmat(struct('mode', 'replaceIQ', ...    % accumulate IQ data.
                   'txnum', 1, ...
                   'rcvnum', 1, ...
                   'pagenum',1, ...
                   'regionnum', 1), 1, ne);
...
for j = 1:ne
    ...
    if isequal(ReconInfo(j).txnum,0)
...
switch UIState
   case 1  % Velocity mode
      ...
      Control(1).Parameters = {'Process',1,'method','computeCFIFreqEst'};
      Control(2).Parameters = {'Process',2,'srcData','signedColor','persistMethod','dynamic','persistLevel',persValue};
      ...
      for k = 1:2:length(Process(2).Parameters)
          if strcmp(Process(2).Parameters{k},'srcData'), Process(2).Parameters{k+1} = 'signedColor';
          elseif strcmp(Process(2).Parameters{k},'persistMethod'), Process(2).Parameters{k+1} = 'dynamic';
          elseif strcmp(Process(2).Parameters{k},'persistLevel'), Process(2).Parameters{k+1} = persf;
          end
      end
   case 2  % Power mode
      ...
      Resource.DisplayWindow(1).Colormap = newMap;
      for k = 1:2:length(Process(2).Parameters)
          if strcmp(Process(2).Parameters{k},'persistLevel'), persf = Process(2).Parameters{k+1}; end
      end
      assignin('base','persf',persf);
      persValue = evalin('base','persp');
      Control(1).Parameters = {'Process',1,'method','computeCFIPowerEst'};
      Control(2).Parameters = {'Process',2,'srcData','unsignedColor','persistMethod','simple','persistLevel',persValue};
      Control(3).Parameters = {'DisplayWindow',1,'colormap',newMap};
      Control(4).Parameters = {'ImageBuffer',1,'lastFrame',0};
      set(findobj('tag','UserB1Edit'),'String',num2str(persValue,'%3.0f'));
      set(findobj('tag','UserB1Slider'),'Value',persValue);
      ...
      for k = 1:2:length(Process(2).Parameters)
          if strcmp(Process(2).Parameters{k},'srcData'), Process(2).Parameters{k+1} = 'unsignedColor';
          elseif strcmp(Process(2).Parameters{k},'persistMethod'), Process(2).Parameters{k+1} = 'simple';
          elseif strcmp(Process(2).Parameters{k},'persistLevel'), Process(2).Parameters{k+1} = persValue;
          end
      end
end
...
for k = 1:2:length(Process(1).Parameters)
    if strcmp(Process(1).Parameters{k},'pwrThreshold'), Process(1).Parameters{k+1} = UIValue; end
end
assignin('base','Process',Process);
% Set Control.Command to set Doppler threshold.
Control = evalin('base','Control');
Control.Command = 'set&Run';
Control.Parameters = {'Process',1,'pwrThreshold',UIValue};
assignin('base','Control', Control);
%-UI#3Callback
...
% If PTool window is open, adjust persistLevel1 in Process(2)
hPTool = findobj('tag','ProcessTool');
if ishandle(hPTool),
    hPNum = findobj('tag','processNum');
    if isequal(get(findobj('tag','processNum'),'Value'),2)
