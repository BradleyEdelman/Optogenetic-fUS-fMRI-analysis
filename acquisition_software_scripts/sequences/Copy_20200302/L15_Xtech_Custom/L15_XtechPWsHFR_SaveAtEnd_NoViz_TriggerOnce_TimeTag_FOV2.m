...
SeqControl(3).argument = 160;  %  usecs
SeqControl(4).command = 'timeToNextAcq';  % time between acquisitions
SeqControl(4).argument = 20000;  %  usecs
...
EF(2).Function = text2cell('%timeTag');
...
persistent idx starttimes lags
...
if idx~=1
    ...
    DUR = 1.5;
    pauseDur = DUR - (starttimes(idx)-starttimes(idx-1));
    if pauseDur > 0
        pause(pauseDur)
        TOC = TOC + pauseDur;
    end
    ...
    lags(idx-1) = pauseDur;
    fprintf('frame #%.f, Duration: %.2f, Cumulative: %.2f\n',idx-1,...
        starttimes(idx)-starttimes(idx-1),starttimes(idx)-starttimes(1));
