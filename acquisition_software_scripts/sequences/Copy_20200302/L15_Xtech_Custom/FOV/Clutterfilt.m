...
elseif isfield(Data,'D') % low resolution functional
    ...
    Data.D.IQR = IQSAVE; 
    Data.D.block = [zeros(1,40) repmat([ones(1,8) zeros(1,32)],1,5)];
...
pdiF{1} = pdi;
pdiA{1} = pdiF{1}(:,:,1);
pdiA{1} = 10*log10(pdiA{1}./max(max(pdiA{1})));

% for i = 1:size(pdi,3)
%     pdi(:,:,i) = 10*log10(pdi(:,:,i)./max(max(pdi(:,:,i))));
% end
% figure; imagesc(pdi(:,:,31))
