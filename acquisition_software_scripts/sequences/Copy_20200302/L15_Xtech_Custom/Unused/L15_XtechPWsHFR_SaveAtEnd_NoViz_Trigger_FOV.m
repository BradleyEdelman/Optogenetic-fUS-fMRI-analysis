...
P.numAngle = 12;        % no. of flash angles for compounding
...
% [file,path] = uigetfile('D:\Data\*.mat');
FOV; pause
% if ~isequal(file,0) && ~isequal(path,0)
if exist('Param','var')
%     tmp = load(strcat(path,file));
%     PData(1).PDelta = tmp.P.delta;
%     PData(1).Origin = tmp.P.origin;
%     PData(1).Size = tmp.P.size;
%     P.startDepth = tmp.P.depthstart;
%     P.endDepth = tmp.P.depthend;
...
SeqControl(3).argument = 1/P.prf*1e6;  %  usecs
SeqControl(4).command = 'returnToMatlab';
SeqControl(5).command = 'sync';
SeqControl(5).argument = 0.5e6;   % 1s
...
process=0; % 0 for no process/viz, 1 for filter/viz, 2 for svd/filter/viz
if isequal(process,1)
    % computation of the power Doppler image
    [B,A]=butter(4,.3,'high');    %coefficients for the high pass filter
...
elseif isequal(process,2)
    ...
    IQR2=IQR2(:,:,4:end);           % the first 4 temporal samples are eliminated (filter oscilations)
