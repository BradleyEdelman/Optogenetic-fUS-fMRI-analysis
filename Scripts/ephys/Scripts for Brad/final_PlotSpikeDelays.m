...
for i = 1:size(mouseList,1)
    ...
    if exist(Filename,'file') 
        ...
        nc = inc;
        all = inc;
        for i_ch = 1:size(Spike_Delays,2)
...
%                 end
%             end
            All_Data{i_group,1,i_frequency,i_flat} = [All_Data{i_group,1,i_frequency,i_flat};cell2mat(reshape(input(:,1:16),[],1))];
            All_Data{i_group,i_region,i_frequency,i_flat} = [All_Data{i_group,i_region,i_frequency,i_flat};cell2mat(reshape(input(:,17:32),[],1))];
            All_Data{i_group,1,3,i_flat} = [All_Data{i_group,1,3,i_flat};cell2mat(reshape(input(:,1:16),[],1))];
            All_Data{i_group,i_region,3,i_flat} = [All_Data{i_group,i_region,3,i_flat};cell2mat(reshape(input(:,17:32),[],1))];
        end
        clear Data;
    end
end
...
for ii = 1:length(namelist)
    ...
    % Save the plot
    SaveFilename = fullfile(working_dir,'Figures_Group',sprintf('SpikeDelays_%s.png',namelist{ii}));
    fprintf('Saving: %s\n',SaveFilename);
