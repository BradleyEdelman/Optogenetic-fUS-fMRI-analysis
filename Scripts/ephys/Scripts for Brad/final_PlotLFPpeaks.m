    ...
    for i_frequency = 1:size(All_Data,3)
        ...
        for i_group = 1:size(All_Data,1)
            for i_region = 2:size(All_Data,2)
                ...
                set(gca,'XTickLabel',[-100:50:100]);
                if p < 5
                    if i_ch <= 16
                        title(sprintf('BF Ch.%d (%s)',i_ch,Regions{i_region}));
                    else
        ...
        % Save the plot
        fprintf('Saving: %s',fullfile(working_dir,'Figures_Group','PlotLFPpeaks',sprintf('LFPpeaks_Ch%d_%s.png\n',i_ch,namelist{i_frequency})));
        saveas(Fig1,fullfile(working_dir,'Figures_Group','PlotLFPpeaks',sprintf('LFPpeaks_Ch%d_%s.png',i_ch,namelist{i_frequency})),'png');
%         pause;
        clf;
    end
end