% Sort the spike waveforms from final_CheckWaveforms.m
...
Groups = {'Young_VGLUT2','Young_VGLUT2','Young_VGLUT2','Young_ChAT','Old_ChAT','Old_ChAT','Old_ChAT_APP','Young_ChAT','Young_ChAT','Young_ChAT','Young_VGLUT2','Old_ChAT','Old_ChAT','Young_VGLUT2','Young_VGLUT2','Old_ChAT','Young_VGLUT2','Old_ChAT','Old_ChAT_APP','Young_VGAT','Old_ChAT_APP','Old_ChAT_APP','Young_VGAT','Young_ChAT','Young_ChAT','Young_VGAT','Young_ChAT','Young_ChAT','Young_VGAT','Young_VGAT','Young_VGAT','Young_VGAT','Young_VGAT','Young_VGLUT2'};
...
%%
source_dir = fullfile(working_dir,'Figures_Before_Recheck','Figures_Indiv','CheckWaveforms');
check_unsorted = 0; % Whether you want to look at the unsorted units as well
for i_mouse = 1:length(Mice)
    mouse = Mice{i_mouse};
    group = Groups{i_mouse};
    folder1 = group;
    for i_region = 1:length(Regions)
        ...
        for i_freq = 1:length(Frequencies)
            ...
            for i_recording = 1:length(Recordings)
                ...
                Filename = fullfile(working_dir,mouse,sprintf('BF_%s_%sHz_%s.mat',region,frequency,recording));
        ...
        for i_wf_idx = 1:2:length(wf_idx)
...
%                 end
                ...
                folder3 = '';
                if INC(1,i_channel,i_unit-1) == 1
                    folder3 = 'INC';
                elseif DEC(1,i_channel,i_unit-1) == 1
                    folder3 = 'DEC';
                elseif NC(1,i_channel,i_unit-1) == 1
                    folder3 = 'NC';
                end
                final_folder = sprintf('%s_%s_%s',folder1,folder2,folder3);
                target_dir = fullfile(source_dir,final_folder);
                if ~exist(target_dir)
                    mkdir(target_dir);
                end
                
                source_file = fullfile(source_dir,sprintf('%d.png',i_png));
                movefile(source_file,target_dir);
