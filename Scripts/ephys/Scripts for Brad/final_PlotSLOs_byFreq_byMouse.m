% Compare the SLO counts at different frequencies, averaged across animals
% or across channels
        ...
        if all(cellfun('isempty',All_Data(i_group,i_region,1,:)))
            ...
            for i_mouse = 1:size(All_Data,4)
                if ~isempty(All_Data{i_group,i_region,1,i_mouse})
                    loc = size(tempData,2);
                    tempData{1,loc+1} = All_Data{i_group,i_region,1,i_mouse};
                    tempData{2,loc+1} = All_Data{i_group,i_region,2,i_mouse};
                end
            end
            for i_mouse = 1:size(tempData,2)
                lowfreq = mean(tempData{1,i_mouse},1)';
                highfreq = mean(tempData{2,i_mouse},1)';
                ...
                n_channels = size(lowfreq,1);
                both_avg = [mean(lowfreq,1,'omitnan') mean(highfreq,1)];
                both_std = [std(lowfreq) std(highfreq)];
                both_sem = [both_std(1)/sqrt(n_channels) both_std(2)/sqrt(n_channels)];
...
% Save the plot
SaveFilename = fullfile(working_dir,'Figures_Group','PlotSLOs_byFreq_byMouse.png');
fprintf('Saving: %s\n',SaveFilename);
