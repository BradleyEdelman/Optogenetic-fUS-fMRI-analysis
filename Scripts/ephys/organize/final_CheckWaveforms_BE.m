function final_CheckWaveforms_BE(storage,base_fold,slash)
...
for i_mouse = 1:size(base_fold,1)  
    ...
    for i_stim = 1:size(stim,2)
        ...
        store_fold = [data_fold 'waveform_imgs' slash];
        if ~exist(store_fold,'dir'); mkdir(store_fold); end
        
        if exist(data_file,'file')
            Waveforms = cell(32,26);
            ...
            for i_channel = 1:size(Waveform_mV,2)
                for i_unit = 1:size(Waveform_mV,3)
                    ...
                    Waveforms{i_channel,i_unit} = [Waveforms{i_channel,i_unit};temp];
                end
            end

            % Plot the spike waveforms
            wf_index = ~cellfun('isempty',Waveforms);
            [wf_idx(1,:,:), wf_idx(2,:,:)] = ind2sub(size(wf_index),find(wf_index));
            for i_wf_idx = 1:length(wf_idx)
                i_channel = wf_idx(1,i_wf_idx);
                i_unit = wf_idx(2,i_wf_idx); 
...
%                 end
    ...
    %             fprintf('Plotting/Saving: %s %s_%sHz - Ch.%d Unit %d\n',mouse,region,frequency,i_channel,i_unit-1);
                data = Waveforms{i_channel,i_unit};
                figure(Fig1); hold on
                for i=1:min(100,size(data,1))
                    plot1 = plot(data(i,:));
                    plot1.Color(4) = 0.5;
                end
                title([stim{i_stim} ' mW, Waveform #: ' num2str(i_wf_idx)])
                xlim([1 32]); xticks(1:4:32);
                xlabel('25us steps'); ylabel('mV');
                saveas(Fig1,[store_fold 'unit: ' num2str(i_wf_idx) '.png'],'png');
                cla;
            end
        end
    end
