function fmri_plot_ROI_info(storage,base_fold,slash,param)
...
for i_stim = 1:size(stim,2)
    ...
    if exist(stim_file,'file') && exist(count_file,'file')
        ...
        for i_roi = 1:20
            ...
            StatROIprop{1,3+i_roi} = fmriroi{i_roi};
            ...
            StatROIt{1,3+i_roi} = fmriroi{i_roi};
...
if isunix
    Tprop = ['/home/bradley/Dropbox/fMRI_ROI_prop_' descr '.txt'];
    Tt = ['/home/bradley/Dropbox/fMRI_ROI_Tval_' descr '.txt'];
elseif ispc
    Tprop = ['C:\Users\Brad\Dropbox\fMRI_ROI_prop_' descr '.txt'];
    Tt = ['C:\Users\Brad\Dropbox\fMRI_ROI_Tval_' descr '.txt'];
end
...
set(gca,'ylim',[0 1],'xtick',1:20,'xticklabel',fmriroi); ylabel('Proportion')
...
errorbar(ctr, ydt, zeros(3,20), cell2mat(tval_roi_sem)', zeros(3,20), zeros(3,20), '.k',...
    'capsize',5,'marker','o','markersize',0.1,'linewidth',1)
hBar{2} = bar(ctrs, data); delete(hBar{1})
set(gca,'ylim',[0 11],'xtick',1:20,'xticklabel',fmriroi); ylabel('Average T-val')
