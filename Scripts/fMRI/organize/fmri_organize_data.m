function fmri_organize_data(storage,base_fold,slash,param)
...
for i_stim = 1:size(stim,2)
    ...
    for i_mouse = 1:size(base_fold,1)
        ...
        recon_fold = [storage base_fold{i_mouse}(1:8) slash base_fold{i_mouse}(17:end) slash];
        snr_fold = [recon_fold stim1{i_stim} slash];
        func_file = [snr_fold 'template' slash 'Func.mat'];

        if exist(func_file,'file')
            
            % Load and store individual data file
            load(func_file)
            ind_data{i_mouse} = squeeze(FUNC.Data.smooth.img(:,:,sliceidx,:));
%             ind_data{i_mouse} = squeeze(FUNC.Data.realign.img(:,:,sliceidx,:));
            ...
            % Compile group data
            grp_data = cat(4,grp_data,squeeze(FUNC.Data.smooth.img(:,:,sliceidx,:)));
%             grp_data = cat(4,grp_data,squeeze(FUNC.Data.realign.img(:,:,sliceidx,:)));
