...
for i_stim = 1:size(stim,2)
    ...
    if exist(stim_file,'file') && exist(count_file,'file') && exist(grp_glm_file,'file')
...
% REGRESSION FOR EACH WHOLE REGION
h1 = figure(1);
for i_roi = 1:20
...
%     scatter([0.1 0.5 1.0], mean(reshape(T1,[],3),1),50,'k','filled')
